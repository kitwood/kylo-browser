...
Additional Options:\n
\t-x, --exclude     - Directory or file to exclude from copy
...
class PyRobocopier:
    ...
    def __setargs(self, args, options=None):
        ...
        for option, value in options:
            if option.lower() in ('-s', '--synchronize'):
                ...
                if (value is None):
                    self.__verbosity = 1
                try:
                    self.__verbosity = int(value)
                except:
                    self.__verbosity = value.count('v') + 1
            else:
                if self.__dir1=='':
                    self.__dir1 = option
                elif self.__dir2=='':
        ...
        self.__dcmp = filecmp.dircmp(self.__dir1, self.__dir2, ignore=self.__excludeList)
            ...
            if stat.S_ISREG(st.st_mode):
                ...
                if self.__creatdirs:
                    try:
                        ...
                        shutil.copytree(fulld1, fulld2, ignore=shutil.ignore_patterns(*exclude))
                        self.__numnewdirs += 1
                        if self.__verbosity > 0:
                            print 'Done.'
                    except shutil.Error, e:
